...
const days = (v?: string) => {
    ...
    if (v) {
        console.log("not NAN days", v);
        config.set({ days: addDays(is_NaN(is_NaN(Number(v)))) })
        // const n = Number(v)
        // if (isNaN(n)) {
        //     config.set({ days: addDays(config.get().days) })
        //     console.log("isNaN days", v);
        // } else {
        //     console.log("not NAN days", n);
        // }
    }
    // else
    //     console.log("not days", v);
    //  return daysElapsed(setup.days)
