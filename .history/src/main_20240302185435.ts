...
import { listUserDir } from '@/.history/src/helpers/listUserDir_20240302120950.ts';
...
const main = async () => {
    ...
    try {
        const users = await listUserDir(config.get().usersPath)
        ...
        for await (const u of users) {
            ...
            if (await exists(dir)) {
                ...
                //     await deleteFiles(dir, listLogs)
            }
    ...
    await appLog.exit(`total: usuários: ${config.get().numberOfDeletedFiles} | Arquivos deletados: ${config.get().numberOfDeletedFiles}`)
}
