...
Deno.test("initSetup logsPath test", async () => {
    ...
    assertEquals(config.get().logsPath, `c:\\Users`);
    await initSetup({ logsPath: `` })
    assertEquals(config.get().logsPath, `c:\\Users`);

    await initSetup({ logsPath: `d:/Users` })
    assertEquals(config.get().logsPath, `d:\\Users`);

    await initSetup({ logsPath: `172.31.165.46\\d$/Users` })
    assertEquals(config.get().logsPath, `\\\\172.31.165.46\\d$\\Users`);
});
