...
Deno.test("listUserDir test", async () => {
    ...
    const u = await listUserDir(dir, '2')
    assertEquals(u.length, 1)
    assertEquals(reassemblePath(u[0]), "C:\\dev\\deleta_arquivos\\dir_tests\\2")
    ...
    assertEquals(reassemblePath(us[0]), "C:\\dev\\deleta_arquivos\\dir_tests\\1")
})